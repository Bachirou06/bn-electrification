...
# ── Tabs ──────────────────────────────────────────────────────────────────────
t_map, t_charts, t_rollout, t_targets, t_sites, t_data = st.tabs(
    ["🗺️ Map", "📊 Charts", "📅 Annual Rollout", "🎯 Access Targets", "📍 Site Filtering", "📋 Data"])
    ...
    with mc:
        ...
        if "X_deg" in df.columns and "Y_deg" in df.columns:
            ...
            def build_hover(row):
                ...
                if dist_c and dist_c in row.index:
...
with t_rollout:
    ...
    rrows=[]
    for t in UNELEC:
        tc=df[df["MinimumOverall"]==t]["InvestmentCost"].sum() if "InvestmentCost" in df.columns else 0
        if tc>0:
    ...
    if rrows:
        ...
        if "NumConnections" in df.columns:
            crow=[]
            for t in UNELEC:
                th=df[df["MinimumOverall"]==t]["NumConnections"].sum()
                if th>0:
        ...
        opex_rate = 0.025
        total_capex_val = df["InvestmentCost"].sum() if "InvestmentCost" in df.columns else 0
        ...
        total_hh = df["NumConnections"].sum() if "NumConnections" in df.columns else 0
        total_pop = df["Population"].sum() if "Population" in df.columns else total_hh * 5
        total_sites = len(df[~df["MinimumOverall"].isin(["Already Electrified"])])
        with pm4:
...
with t_targets:
    ...
    st.markdown(
        ...
        '<b>100% by 2040</b>.</div>', unsafe_allow_html=True
    )
